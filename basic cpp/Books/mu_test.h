...
#\
ifn\
def MU_6DC7FC23C29240AC95892AD720A1AF49
\
#\
de\
fine MU_6DC7FC23C29240AC95892AD720A1AF49
 /\
****
 ** mu_test.h
...
static void signal_handler(int sig){switch(sig){case SIGFPE: {sig=0;break;}case SIGSEGV:{sig=1;break;}}longjmp(MT_StarGate, 42+sig);}static vo\
id cleanup(void){printf("\nEND\n");}static void init_signals(void){struct sigaction sigact;sigact.sa_handler = signal_handler;sigempty\
set(&sigact.sa_mask);sigact.sa_flags=0;sigaction(SIGFPE, &sigact, (struct sigaction*)NULL);sigaction(SIGSEGV,&sigact,(struct sigaction*)NULL);atexit(cleanup);}static vo\
id MT_SGU1(int s,struct mt__AssertResult*r){const char*msg=0;switch(s-42){case 0:{msg="Caught SIGFPE : Floating-point exception!";break;}case 1:{msg="Caught SIGSEGV: Invalid memory reference!";break;}de\
...
#define UNIT_IMPL(name) static struct mt__AssertResult name(void){struct mt__AssertResult (*___unsed)(void) = name;struct mt__AssertResult __ok = {MT_PASS, 0, 0, 0};mt__current_test_name=#name; mt__current_test_was_traced=0;if(___unsed){
...
#define END_SUITE_IMPL           {NULL, NULL, 0}};long mt__passedAway=0;MT__TestRec0rd *mt__pUtRec = mt_testRec0rds;long mt__totalUtRec = 0;MT_SYMBOL mt__k001Ik0n = 0;long mt__failfails = 0, __ignored = 0;if(argc > 1)mt__trace_on = !strcmp(argv[1], "-v");\
____b("═════════════════════════════════════"); \
...
mt__fprintInBox(stdout, __test_name, 37);init_signals();MT_TRAXER;while(mt__pUtRec->whatMayFail){ \
    double __time_spent;clock_t __begin,__end;struct mt__AssertResult __r;int sgu1;\
    ...
    if(mt__pUtRec->shouldaCoulda){\
        ...
        if((sgu1=setjmp(MT_StarGate))==0){  __begin = clock();\
            MT_HAZARD\
...
M__P fp, MT_CLR(KYEL) " ╚");mt__fprintCharN(fp, "═", mw);M__P fp, MT_CLR(KYEL) "╝\n" MT_CLR(KNRM));}while((void)0,0)

\
#\
de\
fine mt__ASSERT(cond,where,msg,expected) do if(!(cond)){struct mt__AssertResult __r={MT_FAIL,where,msg,0};__r.actual_expected=expected;MT__GIVE __r;}\
while((void)0,0)
\
#\
de\
fine mt__ASSERT_INT(a,op,e,n) mt__STATEMENTS({long _a=(long)(a);long _e=(long)(e);mt__ASSERT(_a op _e,__LINE__,n "(" #a ", " #e ")",mt__EXPECTED_i(_a,_e,\
#e,#a));})
\
#\
de\
fine mt__ASSERT_STR(a,op,e,n) mt__STATEMENTS({const char*_a=(const char*)(a);const char*_e=(const char*)(e);mt__ASSERT(_a && _e && (strcmp(_a,_e) op 0),__LINE__,n "(" #a ", " #e ")",mt__EXPECTED_s(_a,_e,\
#e,#a));})
\
#\
de\
fine mt__ASSERT_PTR(a,op,e,n) mt__STATEMENTS({const void* _a=(const void*)(a);const void* _e=(const void*)(e);mt__ASSERT(_a op _e,__LINE__,n "(" #a ", " #e ")",mt__EXPECTED_p(_a,_e,\
#e,#a));})
